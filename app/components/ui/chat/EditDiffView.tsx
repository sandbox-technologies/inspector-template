...
type EditDiffViewProps = {
  data: InspectorDataParts['editResult']
  isActive?: boolean
}

const getFileName = (filePath?: string): string => {
  if (!filePath) return 'Edit result'
  const lastSlash = filePath.lastIndexOf('/')
  return lastSlash === -1 ? filePath : filePath.substring(lastSlash + 1)
}

export const EditDiffView: React.FC<EditDiffViewProps> = ({ data, isActive }) => {
  const diffContent = data?.diff ?? ''

  const diffLines = React.useMemo(() => (diffContent ? diffContent.split('\n') : []), [diffContent])

  const path = data?.path
  const linesAdded = data?.linesAdded
  const linesRemoved = data?.linesRemoved
    ...
    <div className="mb-3 rounded-lg border border-neutral-200 bg-white/70 p-3 text-sm dark:border-neutral-800 dark:bg-neutral-900/70">
      <header className="mb-2 flex items-center text-xs text-neutral-600 dark:text-neutral-300">
        <span className={`flex items-center gap-2 ${iconData.colorClass}`}>
          ...
          {!active && newFile && <span className="rounded bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300 px-1 py-0.5">new</span>}
